...
uintptr_t maxDistance = il2cpp::value(_("BaseMelee"), _("maxDistance")); //public float maxDistance;
...
static auto get_mousePosition = reinterpret_cast<vector3 (*)()>(*reinterpret_cast<uintptr_t*>(il2cpp::method(_("Input"), _("GetMouseButton"), 0, _(""), _("UnityEngine"))));
...
public:
	...
	/*void SetRA() {
		if (settings::weapon::legit_recoil) {
			...
			const auto Input = mem::read<uintptr_t>((uintptr_t)this + 0x4e0);
			vector2 RA = { 0.f, 0.f };
			mem::write<vector2>(Input + 0x64, RA);
		}
	}*/
	...
	//void super_eoka(float attackReady, bool wasAiming) {
	//	*reinterpret_cast<bool*>((uintptr_t)this + 0x360) = attackReady;
	//	*reinterpret_cast<float*>((uintptr_t)this + 0x370) = wasAiming;
	//}
...
public:
	int GetUID() {
		return mem::read<int>((int)this + 0x28);
	}
	uintptr_t entity() {
	...
	char* ClassName() {
		return (char*)mem::read<DWORD64>(mem::read<DWORD64>(mem::read<DWORD64>((DWORD64)this + 0x98)) + 0x10);
	}
...
public:
	...
	weapon* GetWeaponInfo(int Id) {
		DWORD64 Inventory = mem::read<DWORD64>((DWORD64)this + 0x688);
		DWORD64 Belt = mem::read<DWORD64>(Inventory + 0x28);
		DWORD64 ItemList = mem::read<DWORD64>(Belt + 0x38);// public List<Item> itemList;
		DWORD64 Items = mem::read<DWORD64>(ItemList + 0x10); //	public List<InventoryItem.Amount> items;
		return (weapon*)mem::read<DWORD64>(Items + 0x20 + (Id * 0x8));
	}
	weapon* GetActiveWeapon() {
		int ActUID = mem::read<int>((int)this + 0x5c8);
	...
	void Giraffe() {
		if (settings::misc::eyeoffset && unity::GetKey(rust::classes::KeyCode::Y)) {
			...
			mem::write<vector3>(playereye + 0x38, vector3(0, 1.5, 0));
		}

	}
	void Admintime()
	{
		auto Client = *reinterpret_cast<uintptr_t*>((uintptr_t)mem::game_assembly_base + 52092664); //ConVar_Admin_c*
		auto pizda = *reinterpret_cast<uintptr_t*>((uintptr_t)Client + 0xB8); //ну тут да можете пастить не маргает лицушка тоже екстернал горобчик ахуенчик
		mem::write<float>(pizda + 0x0, settings::misc::Time);
	}
	...
	void fov() {
		...
		if (zooming) {
			auto convar = *reinterpret_cast<uintptr_t*>((uintptr_t)mem::game_assembly_base + 52110168); //"ConVar_Graphics_c*"
	...
	bool IsMenu() {
		if (!this) return true;
		DWORD64 Input = mem::read<DWORD64>((DWORD64)this + input);
		return !(mem::read<bool>(Input + 0x98));
	}
